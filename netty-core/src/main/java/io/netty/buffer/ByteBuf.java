 ...
 *      |                         |     (CONTENT)    |                  |
 *      +-------------------------+------------------+------------------+
 *      | 0-readerIndex段不能使用     |   可读数据的端段    |   可写数据的段          |
 *      0 <= readerIndex(读取数据开始的位置) <= writerIndex(写数据开始的位置) <= capacity(ByteBuf最大长度)
 * </pre>
 *
...
public abstract class ByteBuf implements ReferenceCounted, Comparable<ByteBuf> {
    ...
    /**
     * 将读的指针保存起来
     * 应用场景：在开始读取数据的时候，将读的指针保存起来，数据读取完成以后调用resetReaderIndex函数，将指针还原
     ...
     * 
     * 将读取数据的指针还原
     * 应用场景：在开始读取数据的时候，调用markReaderIndex函数将读的指针保存起来，数据读取完成以后调用该函数将指针还原
    ...
    /**
     * 保存当前写指针的位置
     * 应用场景：在开始写数据的时候，将写的指针保存起来，数据写完以后调用resetWriterIndex函数，将指针还原
     ...
     * 
     * 还原当前写指针的位置
     * 应用场景：在开始写数据的时候，调用markWriterIndex函数将写的指针保存起来，数据写完以后调用该函数，将指针还原
